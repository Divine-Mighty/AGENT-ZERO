...
> No competitor names, no sourced market-size figures, and no cited industry reports were used to build New Eden's market case — that was a deliberate choice, not an oversight. Every number below is a **placeholder input** meant to show the *shape* of the model and let real data be dropped in later. Nothing here should appear in an actual pitch deck or financial projection without being replaced by sourced, defensible figures.
...
### Model structure (placeholder inputs — replace all)

| Variable | Illustrative placeholder | Notes |
|---|---|---|
| Addressable annual new-dwelling starts (Australia) | `[INSERT — e.g. X00,000 dwellings/yr]` | Needs sourcing from ABS building-approvals data or equivalent; not sourced here. |
| % of starts realistically addressable by modular/prefab (site access, planning, transport radius from Factory 01) | `[INSERT — e.g. 5–15%]` | Placeholder range only. |
| % of that modular-addressable pool willing to pay a premium-brand price point | `[INSERT — e.g. 5–20%]` | This is the real question the brand has to answer — see [[The Thesis]] point 1. |
| Resulting illustrative serviceable unit volume / year | `= row1 × row2 × row3` | Purely mechanical output of placeholder inputs above — do not treat as a forecast. |
| Illustrative average selling price per module (residential) | `[INSERT — TBD from Unit Economics + market positioning]` | See [[Unit Economics]]. |
| Illustrative annual residential revenue | `= volume × ASP` | Sensitivity-test this, don't point-estimate it. |
...
| Variable | Illustrative placeholder | Notes |
|---|---|---|
| Number of Australian state/territory + federal emergency-management agencies as potential buyers | `[INSERT — e.g. 8–10 jurisdictions]` | Structural count, not a demand estimate. |
| Illustrative pods procured per jurisdiction per multi-year budget cycle | `[INSERT — e.g. low single digits to tens]` | Entirely unvalidated — depends on tender outcomes, agency budgets, disaster incidence. |
| Illustrative average selling price per pod (role-configured: Medical / Command / Accommodation / Support) | `[INSERT — TBD from Unit Economics + role-fitout premium]` | Role configuration likely changes ASP meaningfully; model each role separately once real data exists. |
| Illustrative addressable pod volume per year (steady-state, smoothed across lumpy tender timing) | `= jurisdictions × pods per cycle ÷ cycle length` | Smoothing is a modeling convenience — real revenue will be lumpy, not smooth. See [[Risks & Mitigations]]. |
| Illustrative annual disaster-relief revenue | `= volume × ASP` | |
